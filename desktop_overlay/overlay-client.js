/**
 * AURA Universal Overlay Client v1.2
 */
(function (root) {
  ...
  class AuraOverlayClient {
    constructor(options) {
      ...
      this.onContextDetected = options.onContextDetected || function () {};
    }
    ...
    async analyze(sandboxOpts) {
      sandboxOpts = sandboxOpts || {};
      await this.connect();

      const sandbox = buildSandbox(Object.assign({}, sandboxOpts, {
        activeApp: sandboxOpts.activeApp || sandboxOpts.active_app || '',
        windowTitle: sandboxOpts.windowTitle || sandboxOpts.window_title || '',
        accessibilityText: sandboxOpts.accessibilityText || sandboxOpts.accessibility_text || ''
      }));
      ...
      const payload = {
        ...
        sandbox: sandbox,
        // Send base64 screenshot if present directly in sandboxOpts
        screenshot: sandboxOpts.screenshot || sandboxOpts.screenshot_data || null
      };
      ...
      return new Promise(function (resolve, reject) {
        ...
        ws.onmessage = function (event) {
          try {
            ...
            } else if (data.type === 'context_detected') {
              self.onContextDetected(data.detected_items, data.suggestions);
              finish(null, data);
        ...
        ws.onclose = function () {
          if (!settled) {
            finish(new Error('Connection closed before analyze completed'));
          }
        };

        try {
          ws.send(JSON.stringify(payload));
          console.log('[AURA Overlay] Sent analyze request');
        } catch (e) {
          finish(e);
        }
      });
        ...
        ws.onmessage = function (event) {
          try {
            ...
            if (data.type === 'status') {
              ...
              if (!noisy && raw) self.onStatus(raw);
